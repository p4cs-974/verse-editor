...
// localStorage key prefix for per-document user css map
const LS_PREFIX = "md-editor:user-css:";
const RAW_PREFIX = "md-editor:css-raw:";

// Default map structure when none exists
...
function extractBlockFromCss(selector: string, cssText: string): string | null {
  try {
    ...
    // Match optional .verse-preview-content prefix and additional selectors in a list
...
// Extract the base container block `.verse-preview-content { ... }` if present
...
// Build the list of individual managed selectors from Tag options (split by comma)
...
// Remove CSS blocks for our managed selectors from a CSS string (both scoped and unscoped forms)
function stripManagedBlocks(cssText: string, options: TagOption[]): string {
  ...
  for (const sel of sels) {
    ...
    // Matches blocks like: [optional prefix].verse-preview-content <sel> [optional, other selectors], { ... }
    const blockRe = new RegExp(
      `(?:^|\n|\r)\\s*(?:\\.verse-preview-content\\s+)?${selEsc}(?:\\s*,\\s*(?:\\.verse-preview-content\\s+)?[^,{]+)*\\s*\\{[\\s\\S]*?\\}`,
      "g"
    );
    out = out.replace(blockRe, (m) => {
      // Remove the whole matched block; leave a single newline to keep formatting reasonable
      return "\n";
    });
  }
  return out;
...
// Ensure any CSS block selectors are scoped to .verse-preview-content
function scopeCssBlock(block: string): string {
  const i = block.indexOf("{");
  if (i === -1) return block;
  const head = block.slice(0, i).trim();
  const body = block.slice(i); // includes { ... }
  const scoped = head
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
...
export default function StylingSidebar({
  ...
  // Load from localStorage once per document; if empty, try to hydrate from provided cssContent
  useEffect(() => {
    ...
    if (Object.keys(map).length === 0 && cssContent) {
      ...
      // Normalize any scoped blocks previously saved
  ...
  // Update editor value when selection or map changes
  useEffect(() => {
    const current = cssMap[selected] ?? defaultTemplateFor(selected);
    setValue(current);
  }, [selected, cssMap]);

  // Listen to storage events for multi-tab sync of this specific key
  useEffect(() => {
  ...
  // Aggregate CSS from the current map
  ...
  const apply = async () => {
    ...
    if (documentId) {
      try {
        ...
        // Preserve base preview block separately
        const baseBlock = extractBasePreviewBlock(currentRaw) || "";

        // Remove managed blocks and the base block from current before merging
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    // Enter outside CodeMirror applies
    ...
    // Inside CodeMirror: use Cmd/Ctrl+Enter
  ...
  return (
    <aside
      className={cn(
        "w-96 border-l bg-background p-3 flex flex-col shadow-lg",
        className
      )}
    >
      ...
      <div className="flex-1 min-h-0" onKeyDown={onKeyDown}>
...
function defaultTemplateFor(selector: string) {
  // Provide a minimal unscoped rule block for convenience (scoped during aggregation)
  return `${selector} {\n  /* Add styles here */\n}`;
}
